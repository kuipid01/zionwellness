  ...
  return (
    <div className="w-full flex relative overflow-hidden h-[80vh] md:h-[85vh]">
      <div style={sliderStyle} className="flex w-fit h-full">
        {images.map((image) => (
          <div key={image} className="w-screen  h-full">
            <img
              src={image}
      ...
      <div className="flex h-full w-full opacity-40 left-0 bg-black bottom-0 absolute"></div>
      <div className="flex gap-6 left-1/2 -translate-x-1/2 bottom-14 absolute">
        {images.map((img, index) => (
          <div
            key={img}
            onClick={() => setSlideImage(index)}
            className="w-[13px] cursor-pointer rounded-full h-[13px] bg-gray-300"
          ></div>
        ))}
      </div>
      <div
        className="w-full  absolute top-1/2 left-1/2 -translate-y-1/2 text-white flex justify-center text-center
flex-col -translate-x-1/2"
      >
        <h1
          style={textStyle}
          className="text-[38px] tracking-wider leading-[50px] md:text-[41px] font-[300] md:leading-[100px] md:font-[700]"
        >
          The wholesome care, you <br className="flex md:hidden" /> deserve
        </h1>
        <motion.button
          initial={{ scale: 1, backgroundColor: defaultBackgroundColor }}
          whileHover={{
            scale: 1.1,
